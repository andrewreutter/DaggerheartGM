...
const SneakAttack = Rogue['Sneak Attack'];
const RoguesDodge = Rogue["Rogue's Dodge"];

describe("Rogue Sneak Attack.computeModifierEligibility", () => {
  ...
  it("returns true when Rogue is Cloaked", () => {
    ...
    const result = SneakAttack.computeModifierEligibility({ el, activeElements: [], mapConfig: {} });
  ...
  it("returns false when no allies are in Melee range of any adversary and not Cloaked", () => {
    ...
    const result = SneakAttack.computeModifierEligibility({ el, activeElements: [el, ally, adv], mapConfig: {} });
...
describe("Rogue's Dodge.onDamageReceived", () => {
  it("removes Rogue's Dodge modifier when HP loss > 0", () => {
    ...
    RoguesDodge.onDamageReceived({ character, hpLoss: 1, updateActiveElement: updates });
  ...
  it("does NOT remove modifier when hpLoss < 1", () => {
    ...
    const mod = { id: 'rogues-dodge-r1', name: "Rogue's Dodge", mode: 'roll' };
    const character = { instanceId: 'r1', activeModifiers: [mod] };
    RoguesDodge.onDamageReceived({ character, hpLoss: 0, updateActiveElement: updates });
  ...
  it("does nothing when no Rogue's Dodge modifier is present", () => {
    ...
    RoguesDodge.onDamageReceived({ character, hpLoss: 2, updateActiveElement: updates });
    expect(updates).not.toHaveBeenCalled();
  });
});
