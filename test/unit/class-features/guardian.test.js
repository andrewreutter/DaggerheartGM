...
const Unstoppable = Guardian.Unstoppable;
...
describe('Guardian Unstoppable.onHpDealt', () => {
  it('does nothing when no Unstoppable Die chip is present', () => {
    const { el, updates } = makeEntity();
    Unstoppable.onHpDealt({ character: { ...el, updateActiveElement: updates }, hpDealt: 2, updateActiveElement: updates });
  ...
  it('increments d4 → d6 on first HP dealt', () => {
    ...
    Unstoppable.onHpDealt({ character, hpDealt: 1, updateActiveElement: updates });
  ...
  it('increments d6 → d8', () => {
    ...
    const { el, updates } = makeEntity({ activeModifiers: [mod] });
    Unstoppable.onHpDealt({ character: { ...el }, hpDealt: 3, updateActiveElement: updates });
  ...
  it('caps at d10 — no further increment', () => {
    ...
    const { el, updates } = makeEntity({ activeModifiers: [mod] });
    Unstoppable.onHpDealt({ character: { ...el }, hpDealt: 2, updateActiveElement: updates });
    expect(updates).not.toHaveBeenCalled();
  });
  ...
  it('does not fire when hpDealt < 1', () => {
    const mod = { id: 'unstoppable-die-g1', name: 'Unstoppable Die', dice: 'd4', refreshOn: 'longRest' };
    const { el, updates } = makeEntity({ activeModifiers: [mod] });
    Unstoppable.onHpDealt({ character: { ...el }, hpDealt: 0, updateActiveElement: updates });
    expect(updates).not.toHaveBeenCalled();
  });
});

describe('Guardian Unstoppable.modifyPreThresholdDamage', () => {
  ...
  it('returns dmgTotal unchanged when no Unstoppable Die chip', () => {
    ...
    const roll = { damageTotal: 6 };
    expect(Unstoppable.modifyPreThresholdDamage({ target, roll })).toBe(6);
  });
  ...
  it('reduces Severe-tier damage to just below severe threshold', () => {
    ...
    const roll = { damageTotal: 10 };
    expect(Unstoppable.modifyPreThresholdDamage({ target, roll })).toBe(7); // severe - 1 = 7
  });

  it('reduces Major-tier damage to just below major threshold', () => {
    const target = { activeModifiers: [{ id: 'unstoppable-die-g1' }], thresholds };
    const roll = { damageTotal: 5 };
    expect(Unstoppable.modifyPreThresholdDamage({ target, roll })).toBe(3); // major - 1 = 3
  });

  it('reduces Minor-tier damage to 0', () => {
    const target = { activeModifiers: [{ id: 'unstoppable-die-g1' }], thresholds };
    const roll = { damageTotal: 2 };
    expect(Unstoppable.modifyPreThresholdDamage({ target, roll })).toBe(0);
  });

  it('does not affect 0 damage', () => {
    const target = { activeModifiers: [{ id: 'unstoppable-die-g1' }], thresholds };
    const roll = { damageTotal: 0 };
    expect(Unstoppable.modifyPreThresholdDamage({ target, roll })).toBe(0);
  });
});
