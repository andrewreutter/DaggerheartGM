...
describe('Charged', () => {
  it('offers an intent chip when the character is acting', () => {
    const { chips } = runIntent(Charged, {
  ...
  it('does not offer an intent chip on non-attack actions (e.g. trait rolls)', () => {
    const { chips } = runIntent(Charged, {
  ...
  it('does not offer an intent chip when the character is not acting', () => {
    ...
    const { chips } = runIntent({ ...Charged, _ownerInstanceId: 'char-2' }, {
  ...
  it('queues addTemporaryStatMod mutation when chip is toggled on', () => {
    ...
    const chips = collectChips(
      [{ ...Charged, _ownerInstanceId: 'char-1' }],
      'intent',
      table
    );
