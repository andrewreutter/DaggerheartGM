...
**Core table actions:** **Help an Ally** is **Done** on the shared pre-roll strip (not a V2 feature module): other invited players / the GM can queue helper dice that stay outside the actor’s own-pool (names + keep-highest dice). Each helper spends 1 Hope when the result banner is Applied. Feature die-size overrides (Household Friend, Epic Poetry, Enchanted Aid) apply on helper rows. **Group Rolls** is **Done** (checkbox on non-reaction pre-rolls, collaborator **Roll** opens a local reaction pre-roll vs the banner DC, +1 per success / −1 per failure, Skip opt-out, always table-visible). **Tag Team Rolls** is **Done** (mutually exclusive with Group roll; initiator spends 3 Hope + one session slot on Apply of the chosen Duality; partner makes a full action roll, not a reaction; **Use this roll** picks which Duality applies; Hope grants all involved PCs 1 Hope, Fear grants 1 Fear per PC; combined attack damage on a successful Tag Team attack; Camaraderie extra initiation + 2-Hope partner discount). Belt of Unity 3-PC Tag Team is out of scope.
