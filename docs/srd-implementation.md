...
**V2 engine:** All 24 beastforms have per-form modules under `src/features-v2/beastforms/` (merged onto `registry.beastforms` in `beastforms/index.js`); sub-features are appended by `loadCharacterFeatures` while that beastform is active. Druid transform state is stored under **`featureState['classes:srd-cls-druid']`** (shared **`activeBeastform`** / **`evolutionTraitKey`**) with legacy read support for **`featureState.Beastform` / `Evolution`**.
