...
| **Banner bridge: `engineRollDisplayOnly`** | Guardian (Nemesis Hope/Fear), Wizard (die swaps), similar | `swapHopeFear`, `setDie`, etc. display-only — not persisted on Ack. **Note**: `setRollOutcome` on the `action` roll is now persisted (`_v2ActionRollOutcome` / `_v2ReviewChipsConsumed` via `v2-banner-review-meta.js`); Fearless, Midnight-Touched, and Stealth Expertise are fixed. Remaining: `swapHopeFear`, `setDie`, advantage dice. |
