...
| Bouncing        | weapon_properties/Bouncing.js       | Done      | unblock-target1 | reviewAction multi-target chip using `targetSelect: { targets, multi: true }`. Variable `stressCost: (table) => length` of selected targets. `onUse` queues `addDamageRoll` per target with primary weapon damage. `isRangeWithin` filters targets by attack range. |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Doubled Up      | weapon_properties/DoubledUp.js      | Done      | unblock-target1 | reviewAction single-target chip using `targetSelect: (table) => [...]`. Filters to Melee-range actors excluding primary target. `onUse` queues `addDamageRoll` with primary weapon damage. No resource cost. |                                                                                                                                                                                                                                       |           |
...
| ------------------------------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| In-chip target selection for multi-target chips                                | Bouncing (weapon), Doubled Up (weapon) | Bouncing: "Mark 1 or more Stress to hit that many targets…"; Doubled Up: "deal damage to another target within Melee range" | Done        | unblock-target1    | Added `targetSelect` chip property to `chip-system.js`: simple form `(table) => [{id,name}]` for single target, object form `{ targets, multi: true }` for multi-target. `activateChip` stores `selectedTargetIds` in chipState. Added `RANGE_BAND_ORDER` and `isRangeWithin(range, maxRange)` to `table.js` for range-band comparison. Updated feature authoring guide §B and §C.5. Bouncing uses multi-select + variable stressCost; Doubled Up uses single-select with no cost. All 756 tests passing. |
...
*Last updated: 2026-03-21 (unblock-target1) — Added targetSelect chip API for in-chip target selection. Implemented Bouncing (multi-target, variable stress) and Doubled Up (single Melee target) weapon properties. All 756 tests passing.*