...
| Deflecting      | weapon_properties/Deflecting.js     | Needs Fix      | val-wp3 | `temporaryStatMods` supports function values; reviewAction toggle chip with `armorCost: 1` and `temporaryStatMods: { evasion: (table) => table.me?.armor ?? 0 }`. "Available Armor Score" = `table.me.armor` (unmarked slots). | CONV-010: uses `armorCost: 1` but the documented API property is `armorMark`; the engine's `deductChipCosts` does not handle `armorCost`, so the armor slot cost is never deducted. SRD: "mark an Armor Slot" — armor marking is broken. Fix: rename `armorCost` to `armorMark` in chip and test. |           |
...
*Last updated: 2026-03-21 (val-wp3) — Validated Brutal, Burning, Deadly, Healing weapon properties. Deflecting marked Needs Fix (CONV-010: armorCost should be armorMark). All 729 tests passing.*