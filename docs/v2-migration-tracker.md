...
| Bouncing        | weapon_properties/Bouncing.js       | Done      | impl-A1  | `reviewAction` chip with `selectTargets` (multi-select) + variable `stressCost` via feature state. `addDamageRoll` per bounce target using primary weapon damage. |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Doubled Up      | weapon_properties/DoubledUp.js      | Done      | impl-wp-1 | `reviewAction` chip with `selectTargets` (single Melee target). `addDamageRoll` with secondary weapon damage. No extra cost. |                                                                                                                                                                                                                                       |           |
...
| ------------------------------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| In-chip target selection for multi-target chips                                | Bouncing (weapon), Doubled Up (weapon) | Bouncing: "Mark 1 or more Stress to hit that many targets…"; Doubled Up: "deal damage to another target within Melee range" | Done        |                    | Added `selectTargets` chip property (`(table) => Actor[]`) and `multiSelect` boolean to the V2 chip API. `activateChip` stores `selectedTargetIds` in chip state (read via `chip.get('selectedTargetIds')`). Bouncing uses multi-select + variable `stressCost` via feature state + `addDamageRoll` per target. Doubled Up uses single-select in Melee + secondary weapon `addDamageRoll`. Authoring guide updated with full docs and examples. All 745 tests pass. |
