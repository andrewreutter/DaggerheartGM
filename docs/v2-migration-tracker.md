...
| Bouncing        | weapon_properties/Bouncing.js       | Done      | unblock-target-sel | reviewAction toggle chip with `isSelectTarget` (multi-select). Variable `stressCost` = number of selected targets. `addDamageRoll` per target. Engine extended with `isSelectTarget` chip property. |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Doubled Up      | weapon_properties/DoubledUp.js      | Done      | unblock-target-sel | reviewAction toggle chip with `isSelectTarget` (single-select, melee only). `addDamageRoll` for selected target. No additional cost. |                                                                                                                                                                                                                                       |           |
...
| ------------------------------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| In-chip target selection for multi-target chips                                | Bouncing (weapon), Doubled Up (weapon) | Bouncing: "Mark 1 or more Stress to hit that many targets…"; Doubled Up: "deal damage to another target within Melee range" | Open        | unblock-target-sel | Added `isSelectTarget` chip property to the V2 engine (`chip-system.js`). Works like `isSelect` but for combat targets: function returns valid actor array, engine stores `selectedTargetIds` in chip state. Added `multiSelect` chip property for multi-target selection. Implemented Bouncing (variable stress cost, multi-select, addDamageRoll per target) and Doubled Up (no cost, single-select, melee range filter). Updated authoring guide §B.2 and cheatsheet §2. |
