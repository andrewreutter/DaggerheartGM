...
| Danger Sense        | ancestries/Goblin.js   | Done  | val-V6           | Review chip (rest frequency, 1 Stress) when adversary attacks me or ally within Very Close; forces reroll (requires engine support for actual reroll).                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                | Fixed: ally range check expanded from `=== 'veryClose'` to `=== 'melee'                                                                                                                                                                                                                                                                     |
...
| Brutal          | weapon_properties/Brutal.js         | Validated | val-wp3  | Automatic hook (onReviewAction): checks single-die damage rolls for max face; uses table.rollDie() for extra roll; adds result to damage effect. Only simple dN notation triggers (multi-die expressions like 2d6 cannot be decomposed). |                                                                                                                                                                                    | Back-applied CONV-026: scoped dmgEffect.find() to action target. |           |
| Burning         | weapon_properties/Burning.js        | Validated | val-wp3  | Automatic hook (onResolve): counts damage dice showing value 6; marks that many Stress on the action target. |                                                                                                                                                                                    | Back-applied: added type === 'attack' guard. |           |
...
| Deadly          | weapon_properties/Deadly.js         | Validated | val-wp3    | onReviewOutcome: adds +1 HP to target effects with amount >= 3; scoped to `e.target?.instanceId === table.action?.target?.instanceId` | | Back-applied: added type === 'attack' guard. fix-deadly |
| Deflecting      | weapon_properties/Deflecting.js     | Needs Fix      | val-wp3 | `temporaryStatMods` supports function values; reviewAction toggle chip with `armorCost: 1` and `temporaryStatMods: { evasion: (table) => table.me?.armor ?? 0 }`. "Available Armor Score" = `table.me.armor` (unmarked slots). | CONV-010: uses `armorCost: 1` which is not documented; engine reads `armorMark` (see chip-system.js `_queueCosts`). The armor slot is never actually deducted. Fix: rename `armorCost` to `armorMark`; update test assertion to check `armorMark`. |           |
...
| Healing         | weapon_properties/Healing.js        | Validated | val-wp3 | onRest hook: auto-clears 1 HP from weapon owner on any rest (short or long) |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Hot             | weapon_properties/Hot.js            | Validated | val-wp3 | Purely narrative — no mechanical effect |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
| Invigorating    | weapon_properties/Invigorating.js   | Validated | val-wp3  | onResolve: when isActing + isSuccess, rolls d4; on 4 clears 1 Stress from attacker |                                                                                                                                                                                                                                       |           |
| Lifestealing    | weapon_properties/Lifestealing.js   | Needs Fix | val-wp3 | onResolve: when isActing + isSuccess, rolls d6; on 6 clears 1 HP from attacker (auto-selects HP over Stress; post-resolve player choice not yet supported by V2 API) | SRD: "clear a Hit Point **or** clear a Stress" — the "or clear a Stress" choice is unimplemented; description also omits the Stress option. If V2 API cannot support post-resolve choices, consider marking Blocked with a resolution request for post-resolve player-choice API. |                                                                                                                                                                                                                                       |           |
...
| Painful         | weapon_properties/Painful.js        | Validated | val-wp3  | onResolve: when isActing + isSuccess, marks 1 Stress on attacker |                                                                                                                                                                                                                                       |           |
...
| Reloading       | weapon_properties/Reloading.js      | Validated | val-wp3  | onResolve: when isActing, rolls d6; on 1 marks 1 Stress on attacker (fires regardless of success/failure) |                                                                                                                                                                                                                                       |           |
...
*Last updated: 2026-03-21 (val-wp3) — Validated Brutal, Burning, Deadly, Healing, Hot, Invigorating, Painful, Reloading weapon properties. Deflecting Needs Fix (CONV-010: armorCost→armorMark). Lifestealing Needs Fix (SRD: missing "or clear a Stress" choice). All 729 tests passing.*