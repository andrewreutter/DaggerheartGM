...
| Bouncing        | weapon_properties/Bouncing.js       | Done      | impl-A1  | resolve-phase chip with `isTargetSelect` (filters actors within weapon range), `loop: true`, `stressCost: 1`; `onUse` reads weapon die from `table.rolls.damage.dice` and calls `addDamageRoll` for selected target. |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Doubled Up      | weapon_properties/DoubledUp.js      | Done      | impl-wp-1 | resolve-phase chip with `isTargetSelect` (Melee-range actors only); `onUse` reads `table.me.secondaryWeapon.damage` and calls `addDamageRoll` for selected target. No cost, no loop. |                                                                                                                                                                                                                                       |           |
...
| ------------------------------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| In-chip target selection for multi-target chips                                | Bouncing (weapon), Doubled Up (weapon) | Bouncing: "Mark 1 or more Stress to hit that many targets…"; Doubled Up: "deal damage to another target within Melee range" | Done        |                    | Added `isTargetSelect` chip property to the engine (`chip-system.js`): `(table) => Actor[]` returns eligible targets; engine stores `selectedTargetId` in chip state before `onUse`. Added `loop` chip property (boolean, host-interpreted) for repeatable target selection (Bouncing). Documented in feature-authoring-guide.md §B.2. Bouncing and Doubled Up implemented as resolve-phase chips using the new API. |
...
*Last updated: 2026-03-21 (unblock-target-select) — Added `isTargetSelect` + `loop` chip API to engine; implemented Bouncing and Doubled Up weapon properties using the new API. All 745 tests passing.*