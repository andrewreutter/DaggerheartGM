...
| Eruptive        | weapon_properties/Eruptive.js       | Done | impl-wp3 | onResolve: on successful melee attack, rolls d20 reaction for each nearby adversary (Very Close range); narrates half-damage results for GM adjudication. Splash HP marking via narration (V2 API lacks fixed-amount AOE damage pipeline). |                                                                                                                                                                                                                                       |           |
| Grappling       | weapon_properties/Grappling.js      | Done | impl-wp3 | Two reviewAction chips on successful attack (1 Hope each): Restrain (addCondition) or Pull to Melee (move). |                                                                                                                                                                                                                                       |           |
| Greedy          | weapon_properties/Greedy.js         | Done | impl-wp3 | Intent toggle chip (temporaryStatMods: proficiency +1). Gold cost is narrative-only (V2 API has no gold resource). |                                                                                                                                                                                                                                       |           |
...
| Hooked          | weapon_properties/Hooked.js         | Done | impl-wp3 | reviewAction chip on successful attack (free): move target to Melee range. |                                                                                                                                                                                                                                       |           |
...
| Locked On       | weapon_properties/LockedOn.js       | Done | impl-wp3 | onResolve stores locked target on successful attack; onIntent forces Hope outcome + narration on next attack vs same target and clears state. Weapon-specific gating (primary only) not possible without table.action.weaponId. |                                                                                                                                                                                                                                       |           |
