...
| Brutal          | weapon_properties/Brutal.js         | Validated | val-6aef  | Automatic hook (onReviewAction): checks single-die damage rolls for max face; uses table.rollDie() for extra roll; adds result to damage effect. Only simple dN notation triggers (multi-die expressions like 2d6 cannot be decomposed). |                                                                                                                                                                                    | Back-applied CONV-026: scoped dmgEffect.find() to action target. |           |
| Burning         | weapon_properties/Burning.js        | Validated | val-6aef  | Automatic hook (onResolve): counts damage dice showing value 6; marks that many Stress on the action target. |                                                                                                                                                                                    | Back-applied: added type === 'attack' guard. |           |
...
| Deadly          | weapon_properties/Deadly.js         | Validated | val-6aef    | onReviewOutcome: adds +1 HP to target effects with amount >= 3; scoped to `e.target?.instanceId === table.action?.target?.instanceId` | | Back-applied: added type === 'attack' guard. fix-deadly |
| Deflecting      | weapon_properties/Deflecting.js     | Needs Fix      | val-6aef | `temporaryStatMods` supports function values; reviewAction toggle chip with `armorCost: 1` and `temporaryStatMods: { evasion: (table) => table.me?.armor ?? 0 }`. "Available Armor Score" = `table.me.armor` (unmarked slots). | CONV-010: `armorCost: 1` is not a documented V2 API property; engine ignores it so armor cost is never deducted. Must use `armorMark: 1` (the documented chip cost property for marking armor slots). Test also asserts `armorCost` instead of `armorMark`. |           |
...
| Healing         | weapon_properties/Healing.js        | Validated | val-6aef | onRest hook: auto-clears 1 HP from weapon owner on any rest (short or long) |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
*Last updated: 2026-03-21 (val-6aef) — Validated Brutal, Burning, Deadly, Healing weapon properties. Deflecting marked Needs Fix (CONV-010: armorCost not a documented API property, should be armorMark). All 729 tests passing.*