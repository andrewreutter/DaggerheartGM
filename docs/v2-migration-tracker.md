...
| Bouncing        | weapon_properties/Bouncing.js       | Done      | impl-A1  | Engine extended with `selectTargets` chip property and `multiSelect` flag. Chip uses `selectTargets` to return adversaries in range (excluding primary target), `multiSelect: true` for multi-target, dynamic `stressCost` from feature state, and `addDamageRoll` for bounce damage. |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
| Doubled Up      | weapon_properties/DoubledUp.js      | Done      | impl-wp-1 | Uses `selectTargets` to return melee adversaries (excluding primary target), single-select. `onUse` reads secondary weapon damage and queues `addDamageRoll` to the selected target.                                                                                                                                            |                                                                                                                                                                                                                                       |           |
