...
| Brutal          | weapon_properties/Brutal.js         | Validated | val-wv1  | Automatic hook (onReviewAction): checks single-die damage rolls for max face; uses table.rollDie() for extra roll; adds result to damage effect. Only simple dN notation triggers (multi-die expressions like 2d6 cannot be decomposed). |                                                                                                                                                                                    | Back-applied CONV-026: scoped dmgEffect.find() to action target. |           |
| Burning         | weapon_properties/Burning.js        | Validated | val-wv1  | Automatic hook (onResolve): counts damage dice showing value 6; marks that many Stress on the action target. |                                                                                                                                                                                    | Back-applied: added type === 'attack' guard. |           |
...
| Deadly          | weapon_properties/Deadly.js         | Validated | val-wv1    | onReviewOutcome: adds +1 HP to target effects with amount >= 3; scoped to `e.target?.instanceId === table.action?.target?.instanceId` | | Back-applied: added type === 'attack' guard. fix-deadly |
| Deflecting      | weapon_properties/Deflecting.js     | Needs Fix      | val-wv1 | `temporaryStatMods` supports function values; reviewAction toggle chip with `armorCost: 1` and `temporaryStatMods: { evasion: (table) => table.me?.armor ?? 0 }`. "Available Armor Score" = `table.me.armor` (unmarked slots). | CONV-010: `armorCost` is not a documented chip property; the guide documents `armorMark` for marking armor slots. `armorCost` does not appear in the engine code or feature-authoring-guide.md. |           |
...
| Healing         | weapon_properties/Healing.js        | Validated | val-wv1 | onRest hook: auto-clears 1 HP from weapon owner on any rest (short or long) |                                                                                                                                                                                    |                                                                                                                                                                                                                                       |           |
...
*Last updated: 2026-03-21 (val-wv1) — Validated Brutal, Burning, Deadly, Healing weapon properties. Deflecting marked Needs Fix (CONV-010: `armorCost` not a documented chip property, should be `armorMark`). All 729 tests passing.*