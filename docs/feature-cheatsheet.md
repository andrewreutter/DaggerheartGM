...
- **`fearCost` (number or `(table) => number`)** — **Adversary features only:** spend from the GM Fear pool (`table.top.fear`, 0–12). `canPayChipCosts` / `deductChipCosts` use this when `table.me.isAdversary`; bridge applies `spendFear` / `gainFear` mutations with `applyV2BannerMutations(..., { fearCount })` so the pool updates via `set-fear`. PCs continue to use `hopeCost` against their Hope.
...
- **`table.me.rangeFrom(other)`** / **`table.me.isWithinRangeBandOf(other, 'close')`** / **`table.me.actorsWithinRangeBand('close')`** — Map range on **`Actor`** (`src/features-v2/engine/table.js`). **`actorsWithinRangeBand`** returns **other** `Actor`s on **`activeElements`** within that band or closer (excludes `table.me`).
- **`table.sheet.actionRoll({ rollText, displayName?, rollMeta? })`** — Queues **`sheetActionRoll`**; the GM **`handleV2ReviewChip`** path strips it (see **`stripV2BannerAuxiliaryMutations`** in **`table-ops.js`**) and calls **`postRoll`** so dice hit the table. Pair with **`table.me.actionLoop(title, description)`** for narration-style follow-ups.
- **`runOnVttDamageApplyReviewOutcome: true`** — Opt-in on a registry row so **`hooks.onReviewOutcome`** runs during **damage commit** (`runV2DamageApplyReviewOutcomePhase` in **`v2-action-loop-bridge.js`**) with hydrated **`table.action.effects`** (e.g. **Acidic Form** rider on a successful attack).
