...
- **`table.source`**: Registry row the active feature was collected from (class, weapon, armor, …). **Prefer this** for tier, damage strings, and other registry fields. Shared option-level state: set **`sourceScopeKey`** on the registry row; details in `docs/feature-authoring-guide.md` §2.1 (**CONV-035**). With a scope bag: **`set(key, value)`** marks the key for the Game Table **Feature state** sidebar; **`setInternal(key, value)`** persists without that (framework/mechanical writes). Same for **`table.feature`**.
