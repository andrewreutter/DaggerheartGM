...
| **isSelectTarget** (chip)   | `(table) => [actor]`. Returns valid target actors for in-chip target selection. Engine stores selections as `chip.get('selectedTargetIds')` (string[]). Combine with `multiSelect: true` for multi-target (e.g. Bouncing). Single-select by default.                                                                             | Any (on chip descriptor).                                                                       |
| **multiSelect** (chip)      | `true` or number. When true, target picker allows multiple selections; when number, caps at that count. Only meaningful with `isSelectTarget`.                                                                                                                                                                                   | Any (on chip descriptor).                                                                       |
