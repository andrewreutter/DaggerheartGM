...
| 14 | **Rest moves** | **`getRestMovesForCharacter`** with merged **`_v2RestSlotStats`** (CONV-011); **`placement: 'rest'`** chips via **`collectV2RestPlacementChipsForCharacter`** / **`activateV2RestPlacementChip`** (`v2-action-loop-bridge.js`); Rest banner UI in **`DiceRoller`**; **`wrapEntity`** in ack | Hybrid | Full `hooks.onRest` parity where still missing; more consumables/features on `'rest'` as needed |
