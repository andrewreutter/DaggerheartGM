/**
 * Orderborne community builder.
 *
 * Features:
 *   Dedicated — Record three sayings/values; once per rest, when embodying one, roll d20 as Hope Die (onAct chip).
 *
 * Chip uses requestHopeDieUpgrade: true so the client (GMTableView) calls postHopeDieUpgrade when the chip
 * is used. We do not import api.js here because this module is loaded on the server (via character-calc →
 * registry → communities), and api.js depends on the browser-only 'firebase' package.
 */
export default {
  name: 'Orderborne',
      ...
      'Record three sayings or values your upbringing instilled in you. Once per rest, when you describe how you\'re embodying one of these principles through your current action, you can roll a **d20** as your Hope Die.',
      {
        ...
        onAct(ctx) {
          ...
          canvas.addChip({
            ...
            resetsOn: 'rest',
            onUse(r) {
              r.rollText = r.rollText.replace(/Hope \[d12\]/, 'Hope [d20] (Dedicated)');
            },
          });
        },
      }
