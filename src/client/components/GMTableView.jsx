...
import {
  ...
  clearTableIntent,
  postTableIntentHelp,
  postTableIntentGroup,
  postTableIntentTagTeam,
  postBannerTagTeamChoose,
...
export function GMTableView({ tableId, activeElements, updateActiveElement: pushTableElementUpdate, removeActiveElement, updateActiveElementsBaseData, data, saveItem, saveImage, addToTable, addElements, sendDoAddToTable, onMergeAdversary, user, route, navigate, featureCountdowns = {}, sessionCountdowns = [], updateCountdown, partySize = 1, partyTier = 1, characters = [], tableBattleMods, nextScenes = [], setTableBattleMods, fearCount = 0, setFearCount, partyLoot: partyLootProp, onSetPartyLoot, onMoveInventoryItem, spotlight = null, onSpotlightChange, conditionsHistory = [], onAddConditionsHistoryEntry, onRemoveConditionsHistoryEntry, tableName = '', gmDisplayName = '', tableStateReady = false, onTableNameChange, onDeleteTable, ensureAdventuresLoaded, ensureCharactersLoaded, clearTable, isPlayer = false, isSpectator = false, isPublic = false, onPublicChange, audienceOnlineCount = 0, playerEmail, connectedPlayers = [], playerEmails = [], playerNames = {}, inviteLink = null, onGenerateInviteLink, onRevokeInviteLink, onRemovePlayerEmail, onLeaveTable, gmUid, onPlayerAddCharacter, pendingBanners = [], pendingIntent = null, spotlightRequestGrant = null, onSpotlightRequestGrantConsumed, onFeatureRequestSuccess, onFeatureRequestCancel, rangerFocusRequestedBannerIds, onRangerFocusRerollRequestSuccess, onRangerFocusRerollRequestCancel, previewAsPlayerEmail = null, onPreviewAsPlayer, onExitPreview, actionLog = [], setActionLog, mapConfig, maps = [], activeMapId = null, gmMapView = null, onSetActiveMap, onAddMap, onAddMapWithImage, onRemoveMap, onRenameMap, onMapConfigChange, onMapViewSync, lifeSupportSelections = {}, onLifeSupportSelect, onLifeSupportClear, restMovesSelections = {}, onRestMoveSelect, onRestMoveClear, tableFeatureState = {}, sessionPlayAllowed = true, sessionStarted = true, sessionPaused = false, mapPings = [], onDismissMapPing = () => {}, appendMapPing = () => {},
  ...
  const panelChromeHover = useCallback((kind, payload, extra) => (
    tablePanelChromeHoverProps(
      setPanelChromeHint,
      tablePanelHoverHintModel(kind, payload, { isPlayer, isTouch, ...extra }),
    )
  ), [isPlayer, isTouch]);
  ...
  const [preRollGroupOn, setPreRollGroupOn] = useState(false);
  const [preRollGroupMembers, setPreRollGroupMembers] = useState([]);
  const preRollGroupMembersRef = useRef([]);
  preRollGroupMembersRef.current = preRollGroupMembers;
  const [preRollTagTeamOn, setPreRollTagTeamOn] = useState(false);
  const [preRollTagTeamPartner, setPreRollTagTeamPartner] = useState(null);
  const preRollTagTeamPartnerRef = useRef(null);
  preRollTagTeamPartnerRef.current = preRollTagTeamPartner;
  ...
  const setPreRollAdvantages = (next) => {
    preRollAdvantagesRef.current = next;
    setPreRollAdvantagesState(next);
  };
  ...
  const handleBannerAcknowledge = async (bannerId, roll, options = {}) => {
    ...
    } else if (roll._tagTeamChosen) {
      const initiatorId = roll._tagTeamInitiatorInstanceId || roll._attackerInstanceId;
      const partnerId = roll._tagTeamPartnerInstanceId;
      const involved = [initiatorId, partnerId].filter(Boolean);
      const planned = planTagTeamAckEffects({
        dominant: effectiveDominant,
        involvedInstanceIds: involved,
        initiatorInstanceId: initiatorId,
        hopeCost: roll._tagTeamHopeCost,
        elements: activeElements,
      });
      for (const u of planned.updates) {
        const { instanceId, ...patch } = u;
        if (instanceId) updateActiveElement(instanceId, patch);
      }
      if (planned.fearDelta && setFearCount) {
        const delta = planned.fearDelta;
        setFearCount((prev) => Math.max(0, Math.min(12, (prev ?? 0) + delta)));
      }
    } else if (!isReactionRoll(roll) && !roll._tagTeamIntentId) {
  ...
  const runSessionStartClear = () => {
    ...
    for (const char of charactersList) {
      ...
      if ((char.tagTeamInitiationsUsedThisSession || 0) > 0) {
        updates.tagTeamInitiationsUsedThisSession = 0;
      }
      if (Object.keys(updates).length > 0) {
  ...
  const openReactionCallPanel = useCallback((seedInstanceIds = []) => {
    trackerOverlay.show({
      kind: 'reaction',
      seedInstanceIds: seedInstanceIds.filter(Boolean),
      ...ENCOUNTER_OVERLAY_FALLBACK_RECT,
    });
  }, [trackerOverlay.show]);

  const handleCallReaction = ({ targetInstanceIds, trait, difficulty, traitOverrides }) => {
    const ids = (targetInstanceIds || []).filter(Boolean);
    if (ids.length === 0) return;
    const byId = traitOverrides && typeof traitOverrides === 'object' ? traitOverrides : {};
    const hasOverrides = Object.keys(byId).length > 0;
    handleActionNotification({
      _action: true,
      _reactionCall: true,
      ...
      ...(hasOverrides ? { _reactionTraitByInstanceId: byId } : {}),
    });
  ...
  const clearInitiatorPreRollLocalState = () => {
  ...
  const resetPreRollLocalState = () => {
    clearInitiatorPreRollLocalState();
  ...
  const handlePlayerOwnRoll = (rollText, displayName, rollMeta = {}, context = null) => {
    ...
    if (rollMeta._skipPreRollIntent) {
      dismissAllHoverCards();
      return postRoll(rollText, displayName || rollText, targetTableId, meta)
    ...
    const onProceed = async () => {
      const livePending = preRollBannerRef.current?.pending || pending;
      const finalText = rollWrapper.getFinalRollText()
        + formatHelpAllyRollSuffix(livePending.meta?._helpAlly)
        + formatGroupRollBonus(livePending.meta?._groupRoll?.modifier);
      const metaWithLabel = {
        ...livePending.meta,
        ...(livePending.rollBonusLabel ? { _staticModifierLabel: livePending.rollBonusLabel } : {}),
        ...(!isPlayer && tableId ? { tableId } : {}),
      };
      try {
        await postRoll(finalText, livePending.displayName || livePending.rollText, targetTableId, metaWithLabel);
        if (livePending.meta?._tagTeamRole === 'initiator' || (livePending.meta?._tagTeamIntentId && livePending.meta?._tagTeamRole !== 'partner')) {
          if (livePending.meta?._tagTeamIntentId) {
            tagTeamInitiatorConsumedRef.current = livePending.meta._tagTeamIntentId;
          }
          clearInitiatorPreRollLocalState();
        } else if (isLocalOverlay) {
          const shared = pendingIntentRef.current;
          if (shared && hydrateSharedPreRollRef.current) {
            hydrateSharedPreRollRef.current(shared);
          } else {
    ...
    setPreRollBanner({
      ...
      getFeatureStateFor,
      pending,
      needsDifficulty: isLocalOverlay ? false : needsDifficulty,
      requiresGmDifficulty: isLocalOverlay ? false : needsDifficulty,
      intentId,
      openedByRole,
  ...
  const handleReactionProceed = (instanceId, marqueeRoll) => {
    ...
    const trait = resolveReactionCallTrait(marqueeRoll, instanceId);
    if (!trait) return;
  ...
  const handleGroupRollReaction = (instanceId) => {
    try {
      const intent = pendingIntent;
      if (!intent?._groupRoll || !intent.intentId) return;
      const member = pickGroupMemberRow(
        instanceId,
        preRollGroupMembersRef.current,
        intent.groupMembers,
      );
      if (!member || member.status !== 'pending' || !member.trait) return;
      const characterEl = activeElements.find((e) => e.instanceId === instanceId && e.elementType === 'character');
      if (!characterEl) return;
      const viewer = isPlayer
        ? { role: 'player', uid: user?.uid, email: playerEmail }
        : { role: 'gm', uid: user?.uid, email: user?.email };
      if (!canEditGroupMember(viewer, instanceId, {
        isGm: !isPlayer,
        memberEl: characterEl,
        activeElements,
      })) return;
      const alreadyRolled = (pendingBanners || []).some(
        (b) => b._groupRollIntentId === intent.intentId && b._attackerInstanceId === instanceId,
      );
      if (alreadyRolled) return;
      const displayChar = characterDisplayByInstanceId?.get?.(instanceId) || characterEl;
      const traits = displayChar?.traits || {};
      const baseScore = traits[member.trait] ?? 0;
      const bf = parseBeastformBonus(displayChar?.activeBeastform?.trait_bonus);
      const traitScore = baseScore + (bf?.stat === member.trait ? bf.bonus : 0);
      const traitLabel = TRAIT_FULL?.[member.trait] || member.trait;
      const actorName = characterDisplayName(displayChar)
        || characterDisplayName(characterEl)
        || groupMemberLabel(member, activeElements);
      const rollText = buildTraitRollText(actorName, member.trait, traitScore, null, 2);
      const displayName = `${actorName} — Reaction (${traitLabel})`;
      const dc = intent.difficulty ?? preRollDifficulty ?? 15;
      handlePlayerOwnRoll(rollText, displayName, {
        _attackerInstanceId: instanceId,
        _traitKey: member.trait,
        _isReaction: true,
        _difficulty: dc,
        _groupRollIntentId: intent.intentId,
        _intentPanelForActionRoll: true,
        _deferExperienceToPreRoll: true,
      }, { characterEl });
    } catch (err) {
  ...
  const clearPreRollBanner = () => {
    if (isLocalGroupReactionPreRoll(preRollBanner) || isLocalTagTeamPartnerPreRoll(preRollBanner)) {
      const shared = pendingIntentRef.current;
      if (shared && hydrateSharedPreRollRef.current) {
        hydrateSharedPreRollRef.current(shared);
        return;
    ...
    if (tableId && id && shouldWriteSharedPreRollIntent(preRollBanner)) {
      void clearTableIntent(tableId, id);
    }
  };
  ...
  const intentViewer = isSpectator
    ? { role: 'spectator', uid: user?.uid, email: user?.email }
    : isPlayer
      ? { role: 'player', uid: user?.uid, email: playerEmail }
      : { role: 'gm', uid: user?.uid, email: user?.email };
  const preRollReadOnly = !!(
    pendingIntent
    && shouldShowPreRollBanner(pendingIntent, intentViewer)
    && !isPreRollBannerInteractive(pendingIntent, intentViewer)
  );
  ...
  const hydrateSharedPreRollFromIntent = (intent) => {
    if (!intent) return;
    const readOnly = !isPreRollBannerInteractive(intent, intentViewer);
    const char = activeElements.find((e) => e.instanceId === intent.characterInstanceId);
    if (!char) return;
    const pending = intent.pending || {
      rollText: intent.rollText || '',
      displayName: intent.characterName || '',
      meta: {},
    };
    if (readOnly) {
      const chips = Array.isArray(intent.chips) ? intent.chips : [];
      setPreRollBanner({
        chips,
        characterEl: char,
        pending,
        needsDifficulty: !!intent.needsDifficulty,
        intentId: intent.intentId,
        openedByRole: intent.openedByRole || 'player',
        initiatorUid: intent._initiatorUid,
        initiatorEmail: intent._initiatorEmail,
      });
      applyRemoteSelection(intent, chips.length);
      return;
    }
    const session = collectPreRollSession({
    ...
    const targetTableId = isPlayer ? tableId : null;
    const onProceed = async () => {
      const livePending = preRollBannerRef.current?.pending || session.pending;
      const finalText = rollWrapper.getFinalRollText()
        + formatHelpAllyRollSuffix(livePending.meta?._helpAlly)
        + formatGroupRollBonus(livePending.meta?._groupRoll?.modifier);
      const metaWithLabel = {
      ...
      postDebugLog({
        ...
        sessionId: 'fd8dd5',
        hypothesisId: 'H-D',
        location: 'GMTableView.jsx:hydrateOnProceed',
        message: 'initiator hydrate postRoll meta',
        data: {
          isPlayer,
          targetTableId,
          tableIdInMeta: metaWithLabel.tableId ?? null,
          _tagTeamIntentId: metaWithLabel._tagTeamIntentId ?? null,
          _tagTeamRole: metaWithLabel._tagTeamRole ?? null,
          _tagTeamPartnerInstanceId: metaWithLabel._tagTeamPartnerInstanceId ?? null,
          sessionIntentId: intent.intentId ?? null,
        },
        timestamp: Date.now(),
      });
      // #endregion
      try {
        await postRoll(finalText, livePending.displayName || livePending.rollText, targetTableId, metaWithLabel);
        if (livePending.meta?._tagTeamRole === 'initiator' || livePending.meta?._tagTeamIntentId) {
          if (intent.intentId) tagTeamInitiatorConsumedRef.current = intent.intentId;
          clearInitiatorPreRollLocalState();
        } else {
          resetPreRollLocalState();
        }
  ...
  useEffect(() => {
    if (!pendingIntent) {
      tagTeamInitiatorConsumedRef.current = null;
      if (
        isLocalGroupReactionPreRoll(preRollBannerRef.current)
        || isLocalTagTeamPartnerPreRoll(preRollBannerRef.current)
      ) {
        resetPreRollLocalState();
      }
      if (
    ...
    if (
      pendingIntent._tagTeam
      && (
        tagTeamInitiatorConsumedRef.current === pendingIntent.intentId
        || shouldHideTagTeamInitiatorPreRoll(pendingIntent)
      )
    ) {
      if (preRollBannerRef.current && !isLocalTagTeamPartnerPreRoll(preRollBannerRef.current)) {
        clearInitiatorPreRollLocalState();
      }
      return;
    }
    if (tagTeamInitiatorConsumedRef.current === pendingIntent.intentId && !pendingIntent._tagTeam) {
      tagTeamInitiatorConsumedRef.current = null;
    }
    if (!shouldShowPreRollBanner(pendingIntent, intentViewer)) {
      if (preRollBannerRef.current) {
        resetPreRollLocalState();
        remoteAckedIntentIdRef.current = null;
      }
      return;
    }
    if (!shouldHydrateSharedIntentOverLocal(preRollBannerRef.current, pendingIntent)) {
      return;
    }
    if (!shouldHydrateSharedIntentOverLocalTagTeam(preRollBannerRef.current, pendingIntent)) {
      return;
    }
    ...
    const readOnly = !isPreRollBannerInteractive(pendingIntent, intentViewer);
    ...
    if (local?.intentId === pendingIntent.intentId) {
      ...
      if (apply) {
        const observerChips = Array.isArray(pendingIntent.chips) ? pendingIntent.chips : [];
        if (readOnly) {
          setPreRollBanner((prev) => (prev ? { ...prev, chips: observerChips } : prev));
          applyRemoteSelection(pendingIntent, observerChips.length);
        } else {
          applyRemoteSelection(pendingIntent, local.chips?.length ?? 0);
        }
      }
      return;
    }
    hydrateSharedPreRollFromIntent(pendingIntent);
  ...
  useEffect(() => {
    ...
    setPreRollGroupOn(!!pendingIntent._groupRoll);
    setPreRollGroupMembers(
      (Array.isArray(pendingIntent.groupMembers) ? pendingIntent.groupMembers : []).map((m) => ({
        ...m,
        name: groupMemberLabel(m, activeElements),
      })),
    );
    setPreRollTagTeamOn(!!pendingIntent._tagTeam);
    setPreRollTagTeamPartner(pendingIntent.tagTeamPartner || null);
  ...
  useEffect(() => {
    ...
    if (preRollReadOnly) return;
    if (!shouldWriteSharedPreRollIntent(preRollBanner)) return;
    if (!shouldWriteSharedPreRollIntentForTagTeam(preRollBanner)) return;
  ...
  const handlePreRollProceed = async (opts = {}) => {
    ...
    const membersSnapshot = preRollGroupMembersRef.current.length
      ? preRollGroupMembersRef.current
      : (Array.isArray(pendingIntent?.groupMembers) ? pendingIntent.groupMembers : []);
    const groupOn = !localOverlay && !!(pendingIntent?._groupRoll || preRollGroupOn);
    if (groupOn && !groupRollsComplete(membersSnapshot)) return;
    const tagTeamPartnerSnapshot = preRollTagTeamPartnerRef.current || pendingIntent?.tagTeamPartner || null;
    ...
    const partnerDisplayEl = tagTeamPartnerSnapshot?.instanceId
      ? (characterDisplayByInstanceId?.get?.(tagTeamPartnerSnapshot.instanceId)
        || activeElements.find((e) => e.instanceId === tagTeamPartnerSnapshot.instanceId)
        || null)
      : null;
    ...
    if (!localOverlay) {
      ...
      if (tagTeamOn && banner.intentId) {
        tagTeamInitiatorConsumedRef.current = banner.intentId;
      }
      if (tableId && banner.intentId && !tagTeamOn) {
    ...
    postDebugLog({
      ...
      sessionId: 'fd8dd5',
      hypothesisId: 'H-A',
      location: 'GMTableView.jsx:handlePreRollProceed',
      message: 'pre-roll proceed before onProceed',
      data: {
        isPartnerSource,
        localOverlay,
        tagTeamOn,
        isPlayer,
        bannerIntentId: banner.intentId ?? null,
        pendingIntentId: pendingIntent?.intentId ?? null,
        pendingIntentTagTeam: !!pendingIntent?._tagTeam,
        pendingMeta: {
          _tagTeamIntentId: pending.meta?._tagTeamIntentId ?? null,
          _tagTeamRole: pending.meta?._tagTeamRole ?? null,
          _tagTeamPartnerInstanceId: pending.meta?._tagTeamPartnerInstanceId ?? null,
        },
      },
      timestamp: Date.now(),
    });
  ...
  }, [preRollBanner, preRollExperienceIndex, preRollCompanionExperienceIndex, preRollReadOnly]);
      ...
      return (
        ...
        >
          <GameTableCharacterListCard
            ...
            onCallReaction={!isPlayer ? (instanceId) => openReactionCallPanel([instanceId]) : undefined}
            cardRootProps={{}}
  ...
  const v2ReviewChipsByRollDbId = useMemo(() => {
    ...
    for (const roll of pendingBanners || []) {
      ...
      try {
        ...
        console.error('[v2] review chips failed for banner', id, err);
      }
    }
    return m;
  }, [
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="w-56 bg-dh-canvas border-r border-dh-border flex flex-col overflow-y-auto shrink-0">
        ...
        <div className="px-2 py-1.5 bg-dh-canvas border-b border-dh-border shrink-0">
          <button
            ...
            {...panelChromeHover('support-table')}
        ...
        <div className="bg-dh-canvas border-b border-dh-border shrink-0">
          ...
          {playersOpen && (
            <div className="px-3 pb-3 space-y-2">
              {!isPlayer && (
                <div {...panelChromeHover('invite-link', { hasLink: !!inviteLink })}>
                ...
                joinedPlayers.map((player) => {
                  ...
                  return (
                    <div
                      ...
                      {...panelChromeHover('player', { name: player.name, email: player.email, online: player.online })}
                    >
                      {player.online && (
          ...
          {audienceOnlineCount > 0 && (
            <div className="px-3 pb-2" {...panelChromeHover('audience')}>
              <button
                type="button"
        ...
        <div {...panelChromeHover('characters-section', { canAdd: !isSpectator })}>
        ...
        <div className="p-2 space-y-3">
          <PartyLootCard
            ...
            chromeHoverProps={panelChromeHover('party-loot')}
            sheetTriggerProps={characterOverlay.triggerProps((e) => ({
              type: 'party',
              top: e.currentTarget.getBoundingClientRect().top,
              bottom: e.currentTarget.getBoundingClientRect().bottom,
          ...
          {!isSpectator && (
          <div {...panelChromeHover('bug-report')}>
          ...
          <DiceRoller
            ...
            preRollSlot={(preRollBanner || tagTeamPartnerBanner) ? (
              ...
              {preRollBanner && <PreRollBanner
                ...
                onFinalize={(value) => {
                  const dc = value ?? preRollDifficulty;
                  writeLastDifficulty(dc);
                  setPreRollDifficulty(dc);
                ...
                proceedDisabled={(isLocalGroupReactionPreRoll(preRollBanner) || isLocalTagTeamPartnerPreRoll(preRollBanner)) ? false : (
                  (isPlayer && !!preRollBanner.needsDifficulty && !preRollDifficultyFinalized && preRollFinalizedGmDifficulty == null)
                  || (!!(pendingIntent?._groupRoll || preRollGroupOn) && !groupRollsComplete(
                    preRollGroupMembers.length ? preRollGroupMembers : (pendingIntent?.groupMembers || [])
                  ))
                )}
                proceedDisabledTitle={(
                  isPlayer && !!preRollBanner.needsDifficulty && !preRollDifficultyFinalized && preRollFinalizedGmDifficulty == null
                )
                  ? 'Waiting for the GM to approve the difficulty'
                ...
                pendingResourceCosts={displayPendingResourceCosts}
                groupRoll={!isLocalGroupReactionPreRoll(preRollBanner) && !isLocalTagTeamPartnerPreRoll(preRollBanner) && !!(pendingIntent?._groupRoll || preRollGroupOn)}
                groupMembers={preRollGroupMembers.length ? preRollGroupMembers : (pendingIntent?.groupMembers || [])}
                groupViewer={intentViewer}
                tagTeam={!isLocalGroupReactionPreRoll(preRollBanner) && !isLocalTagTeamPartnerPreRoll(preRollBanner) && !!(pendingIntent?._tagTeam || preRollTagTeamOn)}
                tagTeamPartner={preRollTagTeamPartner || pendingIntent?.tagTeamPartner || null}
                tagTeamHopeCost={tagTeamInitiatorHopeCost((() => {
                  const pid = (preRollTagTeamPartner || pendingIntent?.tagTeamPartner)?.instanceId;
                  if (!pid) return null;
                  return characterDisplayByInstanceId?.get?.(pid)
                    || activeElements.find((e) => e.instanceId === pid)
                    || null;
                })())}
                tagTeamRemaining={tagTeamInitiationsRemaining(
                  preRollBanner.characterEl,
                  characterDisplayByInstanceId?.get?.(preRollBanner.characterEl?.instanceId) || preRollBanner.characterEl,
                )}
                tagTeamDisabledReason={(() => {
                  const remaining = tagTeamInitiationsRemaining(
                    preRollBanner.characterEl,
                    characterDisplayByInstanceId?.get?.(preRollBanner.characterEl?.instanceId) || preRollBanner.characterEl,
                  );
                  const pid = (preRollTagTeamPartner || pendingIntent?.tagTeamPartner)?.instanceId;
                  const partnerEl = pid
                    ? (characterDisplayByInstanceId?.get?.(pid) || activeElements.find((e) => e.instanceId === pid) || null)
                    : null;
                  const cost = tagTeamInitiatorHopeCost(partnerEl);
                  if (remaining <= 0) return 'Already initiated a Tag Team this session';
                  if (remainingHopeForCharacter(preRollBanner.characterEl, displayPendingResourceCosts) < cost) {
                    return `Need ${cost} Hope`;
                  }
                  return '';
                })()}
                tagTeamViewer={intentViewer}
                onTagTeamToggle={(active) => {
                  setPreRollTagTeamOn(active);
                  if (active) {
                    setPreRollVisibility(ROLL_VISIBILITY_TABLE);
                    setPreRollGroupOn(false);
                    setPreRollGroupMembers([]);
                    ...
                    setPreRollBanner((prev) => {
                      if (!prev?.pending) return prev;
                      const meta = { ...prev.pending.meta };
                      delete meta._tagTeamIntentId;
                      delete meta._tagTeamPartnerInstanceId;
                      prev.pending.meta = meta;
                      return { ...prev };
                    });
                  }
                  if (tableId && preRollBanner.intentId) {
                ...
                onTagTeamPartnerAction={({ action, instanceId, trait }) => {
                  if (action === 'setPartner') {
                    const el = activeElements.find((e) => e.instanceId === instanceId);
                    setPreRollTagTeamPartner(el ? {
                      instanceId,
                  ...
                  } else if (action === 'setTrait') {
                    setPreRollTagTeamPartner((prev) => (
                      prev && prev.instanceId === instanceId && prev.status === 'pending'
                        ? { ...prev, trait }
                        : prev
                    ));
                  }
                ...
                onGroupMemberAction={({ action, instanceId, trait }) => {
                  setPreRollGroupMembers((prev) => prev.map((m) => {
                    if (m.instanceId !== instanceId || m.status !== 'pending') return m;
                    if (action === 'setTrait') return { ...m, trait };
                    if (action === 'skip') return { ...m, status: 'skipped' };
                    return m;
                  }));
                  if (tableId && preRollBanner.intentId) {
                    postTableIntentGroup(tableId, {
                ...
                onGroupMemberRoll={handleGroupRollReaction}
      ...
      {!isPlayer && !isSpectator && <div ref={encounterAsideRef} className="w-56 bg-dh-canvas border-l border-dh-border flex min-h-0 shrink-0 flex-col overflow-hidden">
        <div className="px-2 py-2 bg-dh-canvas border-b border-dh-border sticky top-0 z-10 space-y-2">
          ...
          {!sessionPlayAllowed && !sessionPaused ? (
            <button
              ...
              {...panelChromeHover('start-session')}
            >
            ...
            <>
              <div className="flex items-center gap-1">
                {sessionPlayAllowed ? (
                  ...
                  <button
                    ...
                    {...panelChromeHover('resume-session')}
                  >▶ Resume</button>
                )}
                <button
              ...
              <button
                ...
                {...trackerOverlay.triggerProps(trackerTriggerData('reaction', { seedInstanceIds: [] }))}
                {...panelChromeHover('call-for-reaction')}
              >
                ⚡ Call for Reaction
              </button>
            </>
          ...
          {/* Fear tracker */}
          <div className="rounded-lg border border-dh-strong bg-dh-surface px-2.5 py-2" {...panelChromeHover('fear')}>
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-2">
          ...
          <div className="border-t border-dh-border" role="separator" />
          <div className="space-y-2" {...panelChromeHover('environments-section')}>
          <div className="flex items-center justify-between gap-2">
          ...
          <div className="border-t border-dh-border" role="separator" />
          <div className="space-y-2" {...panelChromeHover('adversaries-section')}>
          <div className="flex items-center justify-between gap-2">
      ...
      {isPlayer && !isSpectator && (
        <div className="w-56 bg-dh-canvas border-l border-dh-border flex flex-col overflow-y-auto shrink-0">
          ...
          {consolidatedElements.some(item => item.kind === 'note' && item.element.visibility !== 'gm') && (
            <div className="space-y-2 border-b border-dh-border px-2 pb-2" {...panelChromeHover('notes-section')}>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-dh-muted">Notes</p>
          ...
          <div className="p-2 space-y-2" {...panelChromeHover('adversaries-section')}>
            {(() => {
    ...
    {modalOpen && (
      <ItemPickerModal
        ...
        partySize={partySize}
        partyTier={partyTier}
        quickPicks={modalOpen === 'scenes' ? nextScenes : undefined}
        onSelect={(item) => {
          void (async () => {
    ...
    <EncounterTrackerOverlay
      ...
      onCallReaction={!isPlayer ? handleCallReaction : undefined}
    />
    <EncounterPotentialAdversaryOverlay
