...
function ResultBanner({ roll, resolved, onAcknowledge, onCancel, targets, getTargetsForRoll, getTargetDisadvantageLabels, onApplyDamage, onApplyVulnerable, onConcussiveKnock, disableDismiss, canApplyDamage = true, onQuickTarget, onDoubledUpTarget, onBouncingTarget, wizardsWithHope = [], onNotThisTime, displayOverridesByRollId, tableCharacters = [], rangerFocusRerollChars = [], onRangerFocusReroll, onRangerFocusRerollRequest, rangerFocusRequestedBannerIds, holdThemOffChars = [], onHoldThemOffToggle, onBannerTargetsChange, lockedOnAutoSuccessRollDbIds = new Set(), wingsOfLightFlyingInstanceIds, onWingsD8Toggle, onWingsD8ToggleRequest, onGetWingsD8Extra, getV2DamageBannerAckNotices, sessionRole, isPlayer = false, currentUserUid = null, onResolveInstantly, onReplayDice, v2ReviewChips = [], onV2ReviewChip, resolveV2ReviewChipPicker, getV2ReviewChipDisableHint, canUseV2ReviewChips, v2PendingMoveInfo = { blocked: false, desiredCondition: '', description: '', featureName: '' }, tagTeamPeerRoll = null, tagTeamViewer = null, onTagTeamChoose, ownedDismissLocked }) {
  ...
  const tagTeamResolved = resolveTagTeamBannerDamage(roll, tagTeamPeerRoll);
  const tagTeamPeerDmg = tagTeamResolved.peerTotal;
  ...
  const tagTeamTypeLocked = !!(tagTeamResolved.needsTypePick && !(tagTeamTypePick || roll._tagTeamDamageType || tagTeamResolved.type));
  const tagTeamShownDmgType = tagTeamTypePick || roll._tagTeamDamageType
    || (tagTeamResolved.needsTypePick ? null : tagTeamResolved.type);
  ...
  // Sub-rolls owned by a live parent session hide Apply/Cancel; leftovers unlock.
  const ownedByReactionCall = ownedDismissLocked != null
    ? !!ownedDismissLocked
    : (roll._reactionCallRollDbId != null || roll._groupRollIntentId != null || tagTeamPending);
        ...
        {(dmg || multiDamage) && (() => {
          ...
          return (
            <div className="flex items-baseline justify-center flex-wrap gap-x-1 mt-1.5 leading-snug">
              ...
              {(tagTeamShownDmgType || (!tagTeamResolved.needsTypePick && firstDmgType)) && (
                <span className="text-sm font-semibold text-red-300/80 ml-0.5">{tagTeamShownDmgType || firstDmgType}</span>
              )}
                            ...
                            <>
                              <Tooltip label={v2MoveBlocksAck ? (v2MoveAckTooltip || '') : selectedDamageTargetIds.length === 0 ? 'Select at least one target' : holdThemOffActive && selectedDamageTargetIds.length >= 2 ? `${primaryActionLabel} (3 Hope)` : primaryActionLabel}>
                                <button
                                  ...
                                  disabled={selectedDamageTargetIds.length === 0 || v2MoveBlocksAck || !!(roll._tagTeamChosen && tagTeamTypeLocked)}
                                  className="px-2 py-0.5 rounded text-[11px] font-semibold border border-dh-strong bg-dh-raised/60 text-dh hover:bg-dh-hover hover:text-dh transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-dh-raised/60"
                                >
                                  {primaryActionLabel}
                                </button>
                            ...
                            <>
                              <Tooltip label={v2MoveBlocksAck ? (v2MoveAckTooltip || '') : filteredTargets.length > 0 && !selectedDamageTargetId ? 'Select a target first' : primaryActionLabel}>
                                <button
                                  ...
                                  disabled={(filteredTargets.length > 0 && !selectedDamageTargetId) || v2MoveBlocksAck || !!(roll._tagTeamChosen && tagTeamTypeLocked)}
                                  className="px-2 py-0.5 rounded text-[11px] font-semibold border border-dh-strong bg-dh-raised/60 text-dh hover:bg-dh-hover hover:text-dh transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-dh-raised/60"
                                >
                                  {primaryActionLabel}
                                </button>
            ...
            {tagTeamPeerRoll ? (
              ...
              <>
                ...
                <button
                  ...
                  disabled={tagTeamTypeLocked}
                  onClick={() => onTagTeamChoose?.(roll, {
                    damageType: tagTeamTypePick || roll._tagTeamDamageType || tagTeamResolved.type || '',
                  })}
                  className="px-2 py-0.5 rounded text-[11px] font-semibold border border-amber-600 bg-amber-900/50 text-amber-100 hover:bg-amber-800/60 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Use this roll
                </button>
              </>
      ...
      {(activeBanners.length > 0 || preRollSlot) && (
        ...
        >
          {activeBanners.map(entry => {
            const ownedDismissLocked = typeof shouldLockOwnedBannerDismiss === 'function'
              ? !!shouldLockOwnedBannerDismiss(entry.roll)
              : shouldHideOwnedBannerDismiss(entry.roll, {
                pendingBanners: activeBanners.map((e) => e.roll),
                liveIntentIds: [],
              });
            return (
            <div
            ...
            {entry.roll._rest ? (
              ...
              <ResultBanner
                ...
                ownedDismissLocked={ownedDismissLocked}
                onAcknowledge={!isPlayer && !ownedDismissLocked ? (opts) => {
                  onBannerAcknowledgeRef.current?.(entry._bannerId, entry.roll, opts);
                  dismissBannerById(entry._bannerId);
                } : undefined}
                onCancel={!isPlayer && !ownedDismissLocked ? () => {
                  onBannerCancelRef.current?.(entry._bannerId, entry.roll);
                  dismissBannerById(entry._bannerId);
                } : undefined}
                targets={targets}
