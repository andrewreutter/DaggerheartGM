...
export function EnvironmentForm({ initial, onSave, onCancel, allItems, featureLibraryPortal }) {
  ...
  const addFeatureFromLibrary = feature => setFormData(fd => ({ ...fd, features: [...fd.features, { ...feature, id: generateId() }] }));

  const featureLibraryEl = allItems && (
    <FeatureLibrary
      items={allItems}
      tier={formData.tier}
  ...
  return (
    <>
      ...
      {featureLibraryEl && featureLibraryPortal
        ? createPortal(featureLibraryEl, featureLibraryPortal)
        : null}
    </>
  );
}
