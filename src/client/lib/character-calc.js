...
import ancestryFeaturesRegistry, { ancestryMap } from '../../features/ancestries/index.js';
...
/**
 * Run all registered ancestry features' onCharacterRender hooks against a character.
 * Returns { statMods, virtualWeapons } — the caller merges these into the character result.
 *
 * @param {object} charData - the partially-built character result (with ancestryFeatures, traits, weapons, etc.)
 * @param {Record<string, object>} [registry] - ancestry features registry (defaults to module-level import)
 * @returns {{ statMods: Array<{stat: string, value: number, source: string}>, virtualWeapons: object[] }}
 */
export function runAncestryRender(charData, registry = ancestryFeaturesRegistry) {
  const featureNames = (charData.ancestryFeatures || []).map(f => f.name);
  const statMods = [];
  ...
  return { statMods, virtualWeapons, weapons: weaponsCopy };
}
...
export function recomputeCharacter(data, srdData) {
  ...
  // Apply ancestry stat mods
  const ancestryMods = { traits: {}, maxHp: 0, evasion: 0 };
