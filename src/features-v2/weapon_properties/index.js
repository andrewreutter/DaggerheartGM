import { Barrier } from './Barrier.js';
import { Bonded } from './Bonded.js';
import { Bouncing } from './Bouncing.js';
import { Brave } from './Brave.js';
import { Brutal } from './Brutal.js';
import { Burning } from './Burning.js';
import { Charged } from './Charged.js';
import { Concussive } from './Concussive.js';
import { Cumbersome } from './Cumbersome.js';
import { Deadly } from './Deadly.js';
import { Deflecting } from './Deflecting.js';
import { Destructive } from './Destructive.js';
import { Devastating } from './Devastating.js';
import { DoubleDuty } from './DoubleDuty.js';
import { DoubledUp } from './DoubledUp.js';
import { Dueling } from './Dueling.js';
import { Eruptive } from './Eruptive.js';
import { Grappling } from './Grappling.js';
import { Greedy } from './Greedy.js';
import { Healing } from './Healing.js';
import { Heavy } from './Heavy.js';
import { Hooked } from './Hooked.js';
import { Hot } from './Hot.js';
import { Invigorating } from './Invigorating.js';
import { Lifestealing } from './Lifestealing.js';
import { LockedOn } from './LockedOn.js';
import { Long } from './Long.js';
import { Lucky } from './Lucky.js';
import { Massive } from './Massive.js';
import { Otherworldly } from './Otherworldly.js';
import { Painful } from './Painful.js';
import { Paired } from './Paired.js';
import { Parry } from './Parry.js';
import { Pompous } from './Pompous.js';
import { Persuasive } from './Persuasive.js';
import { Powerful } from './Powerful.js';
import { Protective } from './Protective.js';
import { Quick } from './Quick.js';
import { Reloading } from './Reloading.js';
import { Reliable } from './Reliable.js';
import { Retractable } from './Retractable.js';
import { Returning } from './Returning.js';
import { Scary } from './Scary.js';
import { SelfCorrecting } from './SelfCorrecting.js';
import { Serrated } from './Serrated.js';
import { Sharpwing } from './Sharpwing.js';
import { Sheltering } from './Sheltering.js';
import { Startling } from './Startling.js';
import { Timebending } from './Timebending.js';
import { Versatile } from './Versatile.js';

export {
  Barrier,
  Bonded,
  Bouncing,
  Brave,
  Brutal,
  Burning,
  Charged,
  Concussive,
  Cumbersome,
  Deadly,
  Deflecting,
  Destructive,
  Devastating,
  DoubleDuty,
  DoubledUp,
  Dueling,
  Eruptive,
  Grappling,
  Greedy,
  Healing,
  Heavy,
  Hooked,
  Hot,
  Invigorating,
  Lifestealing,
  LockedOn,
  Long,
  Lucky,
  Massive,
  Otherworldly,
  Painful,
  Paired,
  Parry,
  Pompous,
  Persuasive,
  Powerful,
  Protective,
  Quick,
  Reloading,
  Reliable,
  Retractable,
  Returning,
  Scary,
  SelfCorrecting,
  Serrated,
  Sharpwing,
  Sheltering,
  Startling,
  Timebending,
  Versatile,
};

export default {
  [Barrier.name]: Barrier,
  [Bonded.name]: Bonded,
  [Bouncing.name]: Bouncing,
  [Brave.name]: Brave,
  [Brutal.name]: Brutal,
  [Burning.name]: Burning,
  [Charged.name]: Charged,
  [Concussive.name]: Concussive,
  [Cumbersome.name]: Cumbersome,
  [Deadly.name]: Deadly,
  [Deflecting.name]: Deflecting,
  [Destructive.name]: Destructive,
  [Devastating.name]: Devastating,
  [DoubleDuty.name]: DoubleDuty,
  [DoubledUp.name]: DoubledUp,
  [Dueling.name]: Dueling,
  [Eruptive.name]: Eruptive,
  [Grappling.name]: Grappling,
  [Greedy.name]: Greedy,
  [Healing.name]: Healing,
  [Heavy.name]: Heavy,
  [Hooked.name]: Hooked,
  [Hot.name]: Hot,
  [Invigorating.name]: Invigorating,
  [Lifestealing.name]: Lifestealing,
  [LockedOn.name]: LockedOn,
  [Long.name]: Long,
  [Lucky.name]: Lucky,
  [Massive.name]: Massive,
  [Otherworldly.name]: Otherworldly,
  [Painful.name]: Painful,
  [Paired.name]: Paired,
  [Parry.name]: Parry,
  [Pompous.name]: Pompous,
  [Persuasive.name]: Persuasive,
  [Powerful.name]: Powerful,
  [Protective.name]: Protective,
  [Quick.name]: Quick,
  [Reloading.name]: Reloading,
  [Reliable.name]: Reliable,
  [Retractable.name]: Retractable,
  [Returning.name]: Returning,
  [Scary.name]: Scary,
  [SelfCorrecting.name]: SelfCorrecting,
  [Serrated.name]: Serrated,
  [Sharpwing.name]: Sharpwing,
  [Sheltering.name]: Sheltering,
  [Startling.name]: Startling,
  [Timebending.name]: Timebending,
  [Versatile.name]: Versatile,
};
